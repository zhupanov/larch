    ...
    for eval_idx, eval_dir in enumerate(sorted(search_dir.glob("eval-*"))):
        ...
        for config_dir in sorted(eval_dir.iterdir()):
            if not config_dir.is_dir():
                continue
            # Skip known non-config directories
            known_non_config = {"inputs", "outputs", "skill-snapshot", "skill"}
            if config_dir.name in known_non_config:
                continue
            if not list(config_dir.glob("run-*")):
                print(f"Warning: {config_dir} has no run-* subdirectories, skipping")
                continue
